...
describe('Testando criar turmas do professor', () => {
    ...
    test('Testando o cancelar turma', async () => {
        ...
        let response = await (0, supertest_1.default)(app_1.default).post('/professor/criarturma').send(novaTurma);
        response = await (0, supertest_1.default)(app_1.default).delete('/professor/cancelar_criar_turma').send(cancelarCriarTurma);
        expect(response.statusCode).toBe(200);
    });
});
